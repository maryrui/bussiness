<template>
    <div class="app-container calendar-list-container">
        <p>
            <el-badge :value="upActiveCount" class="item">
                <el-button size="small" :type="map.status==1 ? 'primary' : ''" @click="swichBtn(1)">上架活动</el-button>
            </el-badge>
            <el-badge :value="downActiveCount" class="item" style="margin-left:15px;">
                <el-button size="small" :type="map.status==3 ? 'primary' : ''" @click="swichBtn(3)">下架活动</el-button>
            </el-badge>
        </p>
        ...
        <el-table fit highlight-current-row style="width:100%;" :data="list">
            <el-table-column label="文章编号" prop="storeActivity_id"></el-table-column>
            <el-table-column label="活动名称" align="center" prop="title"></el-table-column>
            <el-table-column label="活动封面">
                <template slot-scope="scope">
                    <img :src="scope.row.coverImage.path" alt="" style="width:50px;height:50px;">
                </template>
            </el-table-column>
            ...
            <el-table-column label="店铺" align="center" prop="store_name">
                <template slot-scope="scope">
                    {{scope.row.store.store_name}}
                </template>
            </el-table-column>
            <el-table-column label="发布时间" align="center" prop="ctime"></el-table-column>
            <el-table-column label="编辑产品" align="center">
                <template slot-scope="scope">
                 
                        <el-button type="" size="mini" icon="el-icon-edit" type="primary" v-if="isAuth('role:goodsList:edit')" @click="$route.push('/goods/editor/'+scope.row.storeActivity_id)">编辑</el-button>

                    <p v-if="isAuth('role:goodsList:status')">
                        <el-button type="danger" size="mini" @click="putawayBtn(scope.$index,scope.row,3)" v-show="map.status==1">下架</el-button>
                        <el-button type="success" size="mini" @click="putawayBtn(scope.$index,scope.row,1)" v-show="map.status==3">上架</el-button>
                    </p>
                </template>
            </el-table-column>
